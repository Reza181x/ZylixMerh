...
def get_user_data(user_id):
    uid = str(user_id)
    if uid not in users_db:
        # Auto-detect owner by ID
        is_owner_flag = (OWNER_ID and str(user_id) == str(OWNER_ID))
        users_db[uid] = {
            "tier": "owner" if is_owner_flag else "free",
            "expiry": None,
            "daily_used": 0,
...
def handle_callback(call):
    ...
    # Force owner tier before any check
    if is_owner(user):
        udata = get_user_data(user.id)
        if udata.get("tier") != "owner":
            udata["tier"] = "owner"
            save_users(users_db)
...
def handle_message(message):
    ...
    # Auto-register new users + force owner tier
    udata = get_user_data(user.id)
    if is_owner(user) and udata.get("tier") != "owner":
        udata["tier"] = "owner"
        save_users(users_db)
